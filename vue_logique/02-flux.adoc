...
=== Description
1. L’utilisateur signale un message.
2. Le service `Messagerie` publie un événement `message.reported`.
3. Le service `Modération` reçoit l’événement et traite le signalement.
4. Le service `Notifications` informe les modérateurs.
...
sequenceDiagram
    ...
    Client->>Messagerie: POST /messages/:id/report
    Messagerie->>Messagerie: Emit "message.reported"
    Messagerie->>Modération: Receive event
    Modération->>Modération: Process report
    Modération->>Notifications: Emit "report_processed"
    Notifications-->>Client: Notification in-app
----
